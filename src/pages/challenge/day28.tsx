...
const lineAni = [
	...
	50%, 100% {
		transform: translate3d(-60px,0,0) scale(2) scaleX(-1);
		opacity: 0;
	}
    `,
    keyframes`
...
`
const Line = styled.svg<IndexProps>`
    fill: none;
    stroke: #fff;
    stroke-width: 2;
    ...
    ${props => 
        css`
            :nth-of-type(${props.index+1}){
                animation: ${lineAni[props.index]} ${speed} ease-out infinite both;
            }
        `
    }
    ...
    return(
        <Frame>
            {Array.from({length: 6}).map((_, index) => (
                <Line key={index} index={index} width="14px" height="60px" >
                    <path d="M2,58 C14,45 14,15 2,2"/>
                </Line>
            ))}
            <Bell width="50px" height="54px" viewBox="0 0 50 54">
